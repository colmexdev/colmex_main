    ...
    for i in 1..slides
      ...
      while k % 5 != 0
        ...
        if j < @resultado.each.length
          ...
          bloque_html = bloque_html + l(Time.parse(fila["horaInicio"]), format: "%H:%M") + "<br>"
          bloque_html = bloque_html + l(Date.strptime(fila["fechaFin"],"%d/%m/%Y"), format: :long) + " | " + l(Time.parse(fila["horaFin"]), format: "%H:%M") + "<br>"
