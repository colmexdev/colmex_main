...
<br><span>' + fila["institucionSede"].to_s + "</span></p></div></div>"
  ...
  def construir_descubre(descubres)
    ...
    descubres.each do |d|
      ...
      if d.imagen != nil
        bloque_html = bloque_html + ActionController::Base.helpers.image_tag(d.imagen.url, :class => "imagen-descubre")
      end
      ...
      bloque_html = bloque_html + "<br /><span>" + 
      bloque_html = bloque_html + d.imagen.url + "</span>" ActionController::Base.helpers.image_tag(Content.where(tipo: d.contenido).first.icono.url, :class => "icono-descubre")
