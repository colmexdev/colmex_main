    ...
    for i in 1..slides
      ...
      while k % 5 != 0
        ...
        if j < @resultado.each.length
          ...
          bloque_html = bloque_html + l(Date.parse fila["fechaInicio"]) + "<br>"
          bloque_html = bloque_html + fila["horaInicio"] + "<br>"
          bloque_html = bloque_html + l(Date.parse fila["fechaFin"]) + "<br>"
