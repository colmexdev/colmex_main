module PrincipalHelper
  ...
  def construye_slider(sliders)
    ...
    sliders.each do |im|
      ...
      html_block = html_block + image_tag(im.imagen.url, :class => "imagen_slider_fondo") + "<div class=\"ghost-slider\">" + (!im.liga.nil? ? link_to(image_tag(im.badge.url, {:class => "imagen_slider_sobre " + im.posicion.downcase}),im.liga, :class => "liga_img") : image_tag(im.badge.url, {:class => "imagen_slider_sobre " + im.posicion.downcase}) ) + "</div>"
