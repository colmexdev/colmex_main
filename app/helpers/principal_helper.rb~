module PrincipalHelper
  ...
  def construye_slider(sliders)
    ...
    sliders.each do |im|
      ...
      html_block = html_block + image_tag(im.imagen.url, :class => "imagen_slider_fondo") + "<div class=\"ghost-slider\">" + (im.liga.to_s != "" ? ( im.liga.to_s.include?("youtu") ? ("<a href=\"" + im.liga.to_s + "\" data-uk-lightbox data-lightbox-type=\"video\">" + image_tag(((I18n.locale.to_s == "en") ? im.badge_eng.url : im.badge_eng.url), {:class => "imagen_slider_sobre " + im.posicion.mb_chars.downcase, :id => "badge-" + i.to_s}) + "</a>") : link_to(image_tag(im.badge.url, {:class => "imagen_slider_sobre " + im.posicion.mb_chars.downcase, :id => "badge-" + i.to_s}),im.liga, :class => "liga_img", :target => ((im.liga.to_s.include?("agenda") || im.liga.to_s.include?("publicaciones")) ? "_blank" : "" ))) : image_tag(im.badge.url, {:class => "imagen_slider_sobre " + im.posicion.mb_chars.downcase, :id => "badge-" + i.to_s}) ) + "</div>"
