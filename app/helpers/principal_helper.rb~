module PrincipalHelper
  ...
  def construye_slider(sliders)
    ...
    control_block = ""
    sliders.each do |im|
      link = (params[:locale] == "es" ? im.liga.to_s : (!im.link_eng.to_s != "" ? im.link_eng.to_s : im.liga.to_s))
      ...
      html_block = html_block + ActionController::Base.helpers.image_tag(im.imagen.url, :class => "imagen_slider_fondo") + "<div class=\"ghost-slider\">" + (link != "" ? ( link.include?("youtu") ? ("<a href=\"" + link + "\" data-uk-lightbox data-lightbox-type=\"video\">" + ActionController::Base.helpers.image_tag(((params[:locale] == "en") ? im.badge_eng.url : im.badge.url), {:class => "imagen_slider_sobre " + im.posicion.mb_chars.downcase, :id => "badge-" + i.to_s}) + "</a>") : link_to(ActionController::Base.helpers.image_tag(((params[:locale] == "en") ? im.badge_eng.url : im.badge.url), {:class => "imagen_slider_sobre " + im.posicion.mb_chars.downcase, :id => "badge-" + i.to_s}),link, :class => "liga_img", :target => ((!link.include?("www.colmex.mx") || (link.include?("assets"))) ? "_blank" : "" ))) : image_tag((params[:locale] == "es" ? im.badge.url : im.badge_eng.url), {:class => "imagen_slider_sobre " + im.posicion.mb_chars.downcase, :id => "badge-" + i.to_s}) ) + "</div>"
  ...
  def construir_descubre(descubres)
    ...
    descubres.each do |d|
      ...
      if d.imagen.url != "/vacio.jpg"
        ...
        bloque_html = bloque_html + "<p><span class=\"titulo-descubre\">" + d.titulo.gsub(/<(\/)?((p)|(div))>/,"") + "</span></p>"
      end
bloque_html = bloque_html + "<a href=\"" + d.liga + "\" " + (d.contenido.mb_chars.downcase == "video" ? ("data-uk-lightbox data-lightbox-type=\"iframe\"") : "target=\"_blank\"" ) + "><div class=\"frame-hover\" id=\"frame-" + i.to_s + "-hov\">"
      bloque_html = bloque_html + "<p><span class=\"cont-descubre\">" + (params[:locale] == "en" ? Content.where("tipo = ?",d.contenido).first.tipo_eng.mb_chars.upcase : d.contenido.mb_chars.upcase) + "</span></p>"
      bloque_html = bloque_html + "<p><span class=\"titulo-descubre\" id=\"tit-desc-" + i.to_s + "\">" + d.titulo.gsub(/<\/?((p)|(div))>/,"") + "</span></p>"
