module PrincipalHelper
  ...
  def construirEvento(fila, chico = false)
    ...
    bloque_html = bloque_html + ( centros.include?(fila["centroSiglas"].mb_chars.downcase) ? ActionController::Base.helpers.image_tag(fila["centroSiglas"].mb_chars.downcase.split(/ *, */) + ".png", :class => "img_sede") : '') + "</div>"
...
<br><span>' + fila["institucionSede"].to_s + "</span></p></div></div>"
  ...
  def construir_descubre(descubres)
    ...
    descubres.each do |d|
      ...
      bloque_html = bloque_html + "<a href=\""+ d.liga + "\" " + (d.contenido.mb_chars.downcase == "video" ? ("data-uk-lightbox data-lightbox-type=\"iframe\"") : "target=\"_blank\"" ) + "><div class=\"frame-hover\">"
      bloque_html = bloque_html + "<p><span class=\"cont-descubre\">" + d.contenido.mb_chars.upcase + "</span></p>"
      bloque_html = bloque_html + "<p><span class=\"titulo-descubre\">" + d.titulo + "</span></p>"
