...
function reescalarDescubre(){
	for(var i = 0; i < descubres.length; i++){
		...
		$("#frame-"+i).removeClass(descubres[i]["visible"] ? "revanimate" : "animate").addClass(!descubres[i]["visible"] ? "revanimate" : "animate").css("height", ajustarAlturas()/*(descubres[i]["visible"] ? ajustarAlturas() : "0")*/);
	}
}
