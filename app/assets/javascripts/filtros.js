...
function reescalarDescubre(){
	for(var i = 0; i < descubres.length; i++){
		...
		$("#frame-"+i).removeClass(descubres[i]["visible"] ? "revanimate" : "animate").addClass(!descubres[i]["visible"] ? "revanimate" : "animate").css({"height": ajustarAlturas(), "width" : (descubres[i]["visible"] ? "" : 0) }/*(descubres[i]["visible"] ? ajustarAlturas() : "0")*/);
	}
}
